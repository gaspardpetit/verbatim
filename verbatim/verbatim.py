...
from verbatim.transcript.sentences import FastSentenceTokenizer, SentenceTokenizer, SaTSentenceTokenizer
from verbatim.transcript.words import VerbatimWord, VerbatimUtterance
...
class WhisperHistory:
    ...
    @staticmethod
    def confirm_transcript(current_words:List[VerbatimWord], transcript:List[VerbatimWord], prefix:List[VerbatimWord], after_ts:int) -> List[VerbatimWord]:
        c_index = 0
    ...
    def confirm(self, current_words: List[VerbatimWord], prefix: List[VerbatimWord], after_ts: int) -> List[VerbatimWord]:
        return max(
            (self.confirm_transcript(current_words=current_words, transcript=transcript, prefix=prefix, after_ts=after_ts) for transcript in self.transcript_history),
            key=len, default=[]
        )
...
class State:
    ...
    def advance_audio_window(self, offset:int):
        LOG.debug(f"Shifting rolling window by {offset} samples ({samples_to_seconds(offset)}s) to offset {self.window_ts + offset} ({samples_to_seconds(self.window_ts + offset)}s).")
...
class Verbatim:
    ...
    def _guess_language(self, audio:np.array, sample_offset:int, sample_duration:int, lang:List[str]) -> Tuple[str,float]:
        ...
        LOG.info(f"Detecting language using samples {sample_offset}({samples_to_seconds(sample_offset)}) to {sample_offset + sample_duration}({samples_to_seconds(sample_offset + sample_duration)})")
    ...
    def transcribe_window(self) -> Tuple[List[VerbatimWord], List[VerbatimWord]]:
        ...
        confirmed_words = self.state.transcript_candidate_history.confirm(current_words=transcript_words, after_ts=self.state.acknowledged_ts-1, prefix=acknowledged_words_in_window)
        ...
        if len(confirmed_words) > 0:
            ...
            newline = os.linesep
            LOG.debug(f"Transcript:\n{newline.join([''.join([w.word for w in history]) for history in self.state.transcript_candidate_history.transcript_history])}\n{''.join(w.word for w in transcript_words)}\n{''.join(w.word for w in confirmed_words)}")
    ...
    def assign_speaker(self, utterance:VerbatimUtterance, diarization:Annotation):
        ...
        if duration < 1:
            ...
            samples = [ start + duration * 0.25, start + duration * 0.5, start + duration * 0.75]
        else:
            samples = [ start + duration * 0.2, start + duration * 0.3, start + duration * 0.4, start + duration * 0.5, start + duration * 0.6, start + duration * 0.7, start + duration * 0.8 ]
        ...
        if len(votes) != 0:
            best_votes = 0
            best_speaker = ""
            for speaker in votes.keys():
                if votes[speaker] > best_votes:
                    best_speaker = speaker
                    best_votes = votes[speaker]
            return best_speaker
    ...
    def commit_utterances(self, utterances:List[VerbatimUtterance]):
        self.state.transcript.append(utterances=utterances)
    ...
    def transcribe(self) -> List[VerbatimUtterance]:
        ...
        try:
            ...
            for u in self.state.unacknowledged_utterances:
                yield u
