...
def extract_text_and_spk(completions: str, po: PromptOptions, skip_meaningless_speaker: bool = True) -> tuple[str, str]:
...
class JsonUtteranceReader:
    ...
    def generate_data_tuple_for_utt(self, utt: dict[str, str]) -> Generator[tuple[str, str, str], None, None]:
    ...
    def generate_data_tuple_from_range(self, *, utt_id, words, p_speakers, t_speakers, start, end) -> Generator[tuple[str, str, str], None, None]:
        ...
        if len(prompt) <= self.po.emit_input_length and len(target) <= self.po.emit_target_length:
    ...
    *,
    ...
    for completion in utt["completions"]:
        if po.completion_suffix and po.completion_suffix in completion:
            completion = truncate_suffix_and_tailing_text(completion, po.completion_suffix)
        completions_list.append(completion)
    completions = " ".join(completions_list).strip()

    # Extract text and speaker.
    utt[llm_text_field], utt[llm_speaker_field] = extract_text_and_spk(completions, po=po)
...
def transfer_llm_completion(
    ...
    llm_text, llm_speaker = extract_text_and_spk(llm_completion, po=po)
    hyp_text, hyp_speaker = extract_text_and_spk(hyp, po=po)
