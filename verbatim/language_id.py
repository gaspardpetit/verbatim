...
class MmsLanguageIdentifier:
    ...
    def __init__(self, *, model_size_or_path: str, device: str):
        try:
            from transformers.models.auto.feature_extraction_auto import AutoFeatureExtractor  # pylint: disable=import-outside-toplevel
...
def create_language_identifier(config: Config, models: _TranscriberOwnerProtocol) -> LanguageIdentifierProtocol:
    ...
    if backend == "transcriber":
        if (config.transcriber_backend or "").lower() in ("voxtral", "voxtral_mlx"):
