...
def create_audio_source(*,
    ...
    if os.path.splitext(input_source)[-1] != ".wav":
        ...
        return create_audio_source(source_config=source_config, device=device,
    ...
    if not stream:
        ...
        if source_config.diarize is not None:
            source_config.diarization = FileAudioSource.compute_diarization(file_path=input_source,
                rttm_file=source_config.diarization_file, device=device, nb_speakers=source_config.diarize)
