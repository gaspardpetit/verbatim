...
class Diarization:
        ...
        with ProgressHook() as hook:
            if self._use_ami:
                ...
                diarization = self.pipeline(
                    file_path, hook=hook, num_speakers=nb_speakers
                )
    ...
    def diarize_utterance(
        self, u: VerbatimUtterance, window_ts: int, audio: np.array, speaker_embeddings
    ):
        rel_start = max(0, u.start_ts - window_ts)
        rel_end = max(0, u.end_ts - window_ts)
        # compute embedding and compare
        model = Model.from_pretrained(
            checkpoint="pyannote/embedding", use_auth_token=self.huggingface_token
        )
        ...
        audio_segment = torch.tensor(
            audio[rel_start:rel_end].reshape(1, 1, -1), device="cuda"
        )
        ...
        embedding = map_with_specifications(
            inference.model.specifications, __first_sample, output
        )
