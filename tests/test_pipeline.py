...
if __name__ == "__main__":
    ...
    audio_source: AudioSource = create_audio_source(
        input_source="tests/data/init.mp3", device=config.device
    )
    verbatim: Verbatim = Verbatim(config=config)
    with audio_source.open() as audio_stream:
        for utterance, unack_utterances, unconfirmed_words in verbatim.transcribe(
